...
$$ E(Y) = \sum_{y\in \mathcal{S}} y f_Y(y), $$ assuming the sum is
well-defined.
...
probability $\pi$, denoted $Y\sim \mathsf{Bernoulli}(\pi)$, if:
...
-   For a discrete random variable and a function $g$,
    $$E(g(Y))=\sum_{y\in\mathcal{S}}g(y)f_Y(y),$$ assuming the sum is
    well-defined.
-   For a continuous random variable and a function $g$,
    $$E(g(Y))=\int_{y\in\mathcal{S}}g(y)f_Y(y)\;dy,$$ assuming the
...
Using this information and the facts above, we can easily determine the
...
water bottle he uses to stay hydrated through the day and drinks several
...
\end{aligned}
$$ Alternatively, the variance of $\mathbf{y}$ is called the
**covariance matrix** of $\mathbf{y}$ or the **variance-covariance
matrix** of $\mathbf{y}$.
